...
export class VendorsService {
  ...
  async findAll(q?: { search?: string; active?: string }): Promise<Vendor[]> {
  ...
  async create(data: any): Promise<Vendor> {
    const code = (data.vendorCode ?? data.vendor_code ?? '')
      .trim()
      .toUpperCase();
  ...
  async update(id: number, data: any): Promise<Vendor> {
    ...
    if (data.vendorCode !== undefined || data.vendor_code !== undefined) {
      v.vendorCode = String(data.vendorCode ?? data.vendor_code)
        .trim()
        .toUpperCase();
    }
  ...
  async createMapping(data: any): Promise<VendorItemMapping> {
    ...
    const m = this.mapRepo.create({
      ...
      minimumOrderQty: Number(
        data.minimumOrderQty ?? data.minimum_order_qty ?? 0,
      ),
      ...
      lastPurchaseRate:
        data.lastPurchaseRate != null || data.last_purchase_rate != null
  ...
  async updateMapping(id: number, data: any): Promise<VendorItemMapping> {
    ...
    if (
      ...
      data.minimum_order_qty !== undefined
    ) {
      m.minimumOrderQty = Number(
        data.minimumOrderQty ?? data.minimum_order_qty ?? 0,
      );
    }
    ...
    if (
      data.preferredVendor !== undefined ||
      data.preferred_vendor !== undefined
    ) {
      m.preferredVendor = Boolean(
        data.preferredVendor ?? data.preferred_vendor,
      );
    }
    if (
      data.lastPurchaseRate !== undefined ||
      data.last_purchase_rate !== undefined
    ) {
      m.lastPurchaseRate =
        data.lastPurchaseRate ?? data.last_purchase_rate ?? null;
    }
  ...
  async resolveMapping(
    ...
    itemId: number,
    itemSource: string,
  ): Promise<VendorItemMapping | null> {
