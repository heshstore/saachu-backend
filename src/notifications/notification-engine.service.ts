...
import { NotificationType, NotificationPriority, NotificationCategory } from './notification.entity';
...
export class NotificationEngineService {
  ...
  async onWhatsAppDown(payload: { reason: string }): Promise<void> {
    try {
      ...
      await this.notifService.createRoleNotification(['Admin'], {
        ...
        message:         `Reason: ${payload.reason}. ${isCritical
  ...
  async onPaymentReceived(payload: { orderId: number; amount: number; createdBy: number }): Promise<void> {
    const rule     = NOTIFICATION_RULES['payment.received'];
    const managers = await this.userRepo.find({ where: { role: 'Production Manager', is_active: true }, select: ['id'] });
    ...
    try {
      ...
      for (const [key, jobs] of groups) {
        ...
        const maxOverdueHours = Math.max(
          ...jobs.map(j => Math.round((Date.now() - new Date(j.due_date!).getTime()) / 3_600_000)),
        );
    ...
    try {
      ...
      for (const [userId, leads] of byUser) {
        ...
        await this.notifService.createNotification({
          ...
          action_url:      count === 1 ? `/crm/leads/${leads[0].id}` : '/crm/leads?filter=hot',
          cooldownMinutes: 240,
          is_automated:    true,
        });
      }
    ...
    try {
      ...
      for (const user of productionUsers) {
        const [completedToday, pendingTotal] = await Promise.all([
          this.jobRepo.count({ where: { assigned_to: user.id, status: ProductionJobStatus.DONE } }),
          this.jobRepo.count({ where: { assigned_to: user.id, status: In(ACTIVE_STATUSES) } }),
        ]);
  ...
  sendEndOfDayReport(userId: number, summary: { completedToday: number; pendingTotal: number; userName?: string }): void {
