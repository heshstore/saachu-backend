...
export class AnalyticsService implements OnModuleInit {
  ...
  async track(dto: TrackEventDto): Promise<{ success: boolean }> {
    ...
    this.logsService.log(LogAction.ANALYTICS_TRACKED, {
      event: dto.event,
      product: dto.product ?? null,
      session_id: dto.session_id,
    });
  ...
  async getSummary(): Promise<{
    ...
    const rows: { event: string; count: string }[] = await this.dataSource
      .query(`
  ...
  async getTopProducts(): Promise<
    { product: string; total_views: number; whatsapp_clicks: number }[]
  > {
    const rows: { product: string; event: string; count: string }[] = await this
      .dataSource.query(`
    ...
    const map: Record<
      string,
      { total_views: number; whatsapp_clicks: number }
    > = {};
    for (const r of rows) {
      if (!map[r.product])
        map[r.product] = { total_views: 0, whatsapp_clicks: 0 };
      if (r.event === 'product_view')
        map[r.product].total_views += Number(r.count);
      if (r.event === 'whatsapp_click')
        map[r.product].whatsapp_clicks += Number(r.count);
    }
  ...
  async getSourceBreakdown(): Promise<{ source: string; count: number }[]> {
    const rows: { source: string; count: string }[] = await this.dataSource
      .query(`
  ...
  async getOperationalKpis() {
    ...
    const [
      ...
      safe(
        `SELECT COUNT(*)::int AS count FROM quotation WHERE created_at::date = CURRENT_DATE`,
      ),
      safe(
        `SELECT COUNT(*)::int AS count FROM orders WHERE created_at::date = CURRENT_DATE AND status <> 'CANCELLED'`,
      ),
      safe(
      ...
      safe(
        `SELECT COALESCE(SUM(amount),0)::numeric AS total FROM payments WHERE created_at::date = CURRENT_DATE`,
      ),
      safe(
        `SELECT COUNT(*)::int AS count FROM orders WHERE status = 'DISPATCHED' AND pending_amount > 0`,
      ),
      safe(
        `SELECT COUNT(*)::int AS count FROM production_jobs WHERE status IN ('PENDING','IN_PROGRESS')`,
      ),
      safe(
        `SELECT status FROM whatsapp_sessions ORDER BY last_active_at DESC LIMIT 1`,
      ),
      safe(
        `SELECT COUNT(*)::int AS count FROM leads WHERE status NOT IN ('WON','LOST','CANCELLED','REJECTED')`,
      ),
    ...
    const verifiedAt =
      getSyncStatus().lastSuccessfulSyncAt ?? syncRows[0]?.last_sync ?? null;
    if (verifiedAt) {
      shopifySyncMinutes = Math.round(
        (Date.now() - new Date(verifiedAt).getTime()) / 60_000,
      );
    }
  ...
  async getSalesAnalytics(days = 30) {
    ...
    const [quotRows, convRows, leadRows, followupRows] = await Promise.all([
      safe(
        `SELECT COUNT(*)::int AS sent FROM quotation WHERE created_at >= $1`,
        [from],
      ),
      safe(
        `SELECT COUNT(*)::int AS converted FROM quotation WHERE status = 'CONVERTED' AND updated_at >= $1`,
        [from],
      ),
      safe(`SELECT COUNT(*)::int AS total FROM leads WHERE created_at >= $1`, [
        from,
      ]),
      safe(
        `
    ...
    return {
      ...
      followup_completion_rate:
        fuTotal > 0 ? Math.round((fuDone / fuTotal) * 100) : null,
    };
  }
  ...
  async getProductionAnalytics(days = 30) {
    ...
    return {
      ...
      avg_hours:
        s.avg_hours != null ? Number(Number(s.avg_hours).toFixed(1)) : null,
  ...
  async getSystemHealth() {
    ...
    const [dbRows, waRows, syncRows, jobRows, schedulerRows] =
      await Promise.all([
        safe(`SELECT 1 AS ok`),
        safe(
          `SELECT status, last_active_at FROM whatsapp_sessions ORDER BY last_active_at DESC LIMIT 1`,
        ),
        safe(
          `SELECT MAX(updated_at) AS last_sync, COUNT(*)::int AS total FROM shopify_catalog_items`,
        ),
        safe(
          `SELECT COUNT(*) FILTER (WHERE status = 'DONE' AND completed_at::date = CURRENT_DATE)::int AS jobs_today FROM production_jobs`,
        ),
        safe(
          `SELECT COUNT(*)::int AS count FROM kpi_snapshots WHERE created_at >= NOW() - INTERVAL '2 hours'`,
        ),
      ]);
    ...
    let shopifySyncMinutes: number | null = null;
    const verifiedAt2 =
      getSyncStatus().lastSuccessfulSyncAt ?? syncRow?.last_sync ?? null;
    if (verifiedAt2) {
      shopifySyncMinutes = Math.round(
        (Date.now() - new Date(verifiedAt2).getTime()) / 60_000,
      );
    }

    const waConnected =
      waRow?.status === 'AUTHENTICATED' || waRow?.status === 'CONNECTED';
