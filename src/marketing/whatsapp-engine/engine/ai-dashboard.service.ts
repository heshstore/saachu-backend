...
export class AiDashboardService {
  ...
  async getDashboard() {
    ...
    const isInconsistent =
      todayActivity.queue_items > 0 && todayActivity.campaigns_created === 0;
    const warnings = this._computeWarnings(
      todayActivity,
      engineStatus,
      todayQueue,
    );
  ...
  private async _getEngineStatus(todayIso: string) {
    const autoAiMode = await this.engineSettings
      .getAutoAiMode()
      .catch(() => false);
    const allNumbers = await this.numberRepo.find();
    const connectedNumbers = allNumbers.filter(
      (n) =>
    ...
    const lastRunRows = await this.ds
      .query<{ created_at: string; metadata: unknown }[]>(
    ...
    const todayQueueRows = await this.ds
      .query<
        { cnt: string }[]
      >(`SELECT COUNT(*) AS cnt FROM whatsapp_message_queue WHERE created_at >= $1`, [todayIso])
      .catch(() => [{ cnt: '0' }]);
  ...
  private async _getTodayActivity(todayIso: string) {
    ...
    return {
      ...
      campaigns_display:
        queueTotal > 0 && campaignsVal === 0 ? '—' : campaignsVal,
  ...
  // ── Database summary counts (Customer Contacts + Promotional Contacts) ──────

  private async _getDatabaseCounts() {
    const rows = await this.ds
      .query<{ customer_count: string; promotional_count: string }[]>(
        `
      SELECT
  ...
  // ── Audience Status (Promotional Contacts + Eligible Today) ─────────────────
  ...
  private async _getAudienceStatus() {
    ...
    `,
      ...
      .catch(() => [
        { promotional_count: '0', customer_count: '0', eligible_count: '0' },
      ]);
  ...
  private async _getTodayActivityByTelecaller(todayIso: string) {
    ...
    return Promise.all(
      numbers.map(async (n) => {
        const m = await fetchAuthoritativeMetrics(this.ds, since, {
          numberId: n.id,
        });
        return {
          number_id: n.id,
  ...
  private async _getAiCampaigns(todayIso: string) {
    ...
    const replyRows = await this.ds
      .query<{ campaign_id: string; cnt: string }[]>(
        `
    ...
    const replyMap = new Map<string, number>(
      replyRows.map(
        (r) => [r.campaign_id, parseInt(r.cnt, 10)] as [string, number],
      ),
    );

    // Leads per campaign
    const leadRows = await this.ds
      .query<{ campaign_id: string; cnt: string }[]>(
        `
    ...
    const productRows = await this.ds
      .query<
        {
    ...
    for (const p of productRows) {
      if (p.campaign_id)
        productByCampaign.set(p.campaign_id, parseInt(p.cnt, 10));
    ...
    return rows.map((r) => {
      ...
      const promoId =
        r.promo_id ??
        `PROMO-${new Date(r.created_at).toISOString().slice(0, 10).replace(/-/g, '')}-????`;
      const products =
        productByCampaign.get(r.id) ??
        (isToday && r.telecaller_number_id
          ? (productByTelecaller.get(r.telecaller_number_id) ?? 0)
          : 0);
      return {
        id: r.id,
        promo_id: promoId,
  ...
  private async _getTelecallerPerformance(todayIso: string) {
    ...
    const campRows = await this.ds
      .query<
        { telecaller_number_id: string; id: string; promo_id: string | null }[]
      >(
        `
    ...
    const campMap = new Map<string, { id: string; promo_id: string | null }>(
      campRows.map(
        (c) =>
          [c.telecaller_number_id, c] as [
            string,
            { id: string; promo_id: string | null },
          ],
      ),
    );

    const assignedRows = await this.ds
      .query<{ number_id: string; cnt: string }[]>(
        `
      ...
      numbers
        ...
        .map(async (n) => {
          const [m, lastRows] = await Promise.all([
            ...
            this.ds
              .query<{ last_activity: string | null }[]>(
                `
          ...
          return {
            ...
            connected:
              waStatus.number_state === NumberConnectionState.CONNECTED,
            wa_state: waStatus.waState,
            warmup_level: n.warmup_level,
  ...
  private async _getNumberUtilization(todayIso: string) {
    ...
    return Promise.all(
      numbers.map(async (n) => {
        ...
        const metrics = await fetchAuthoritativeMetrics(this.ds, since, {
          numberId: n.id,
        });
        ...
        return {
          ...
          utilization_pct:
            releaseAllowance > 0
              ? Math.round((sentToday / releaseAllowance) * 100)
              : 0,
        };
      }),
    );
  ...
  private async _getLogStream(todayIso: string) {
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        reason: r.reason ?? '',
        created_at: r.created_at,
        log_date: ts.toLocaleDateString('en-IN', {
          day: '2-digit',
          month: 'short',
        }),
        log_time: ts.toLocaleTimeString('en-IN', {
          hour: '2-digit',
          minute: '2-digit',
          second: '2-digit',
          hour12: false,
  ...
  async getQueueInspection(numberId: string, offset = 0, limit = 10) {
    ...
    const [rows, countRows] = await Promise.all([
      this.ds
        ...
        LIMIT $3 OFFSET $4
      `,
          [todayIso, numberId, limit, offset],
        )
        .catch(() => []),
  ...
  private _computeWarnings(
    ...
    if (status.engine_enabled && !status.engine_ran_today && hour >= 9) {
      warnings.push(
        'No AI queue built today — autonomous engine may not have run',
      );
    }
    ...
    if (queue.oldest_pending_minutes > 15 && inSendWindow) {
      warnings.push(
        `Queue stuck — oldest pending item is ${queue.oldest_pending_minutes} minutes old`,
      );
    }
    ...
    if (total >= 5) {
      ...
      if (failRate > 20) {
        warnings.push(
          `High failure rate: ${Math.round(failRate)}% — check number health`,
        );
      }
    }
    return warnings;
