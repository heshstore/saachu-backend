...
export class StabilityReportService {
  ...
  async getReport(days = 14): Promise<StabilityReport> {
    ...
    const [totalsRows, dailyRows, auditRows, audienceRows, numberRows] =
      await Promise.all([
    ...
    const deliveryWarnings = auditRows.filter(
      (r: any) => r.event === 'LOW_DELIVERY_WARNING',
    );
    const readWarnings = auditRows.filter(
      (r: any) => r.event === 'LOW_READ_WARNING',
    );
    const recoveries = auditRows.filter(
      (r: any) => r.event === 'NUMBER_RECOVERED',
    );
    const scaleUps = auditRows.filter((r: any) => r.event === 'SCALE_UP');
    const manualReEnables = auditRows.filter(
      (r: any) => r.event === 'MANUAL_REENABLE',
    );
    const fingerprintSkips = auditRows.filter(
      (r: any) => r.event === 'FINGERPRINT_SKIP',
    );
    const hardLimitHits = auditRows.filter(
      (r: any) => r.event === 'HARD_LIMIT_HIT',
    );
    ...
    if (total > 0 && inCooldown / total > 0.4)
      issues.push(
        `High cooldown ratio: ${Math.round((inCooldown / total) * 100)}% of audience in cooldown`,
      );
    if (total > 0 && highFatigue / total > 0.3)
      issues.push(
        `High fatigue: ${Math.round((highFatigue / total) * 100)}% of audience has fatigue_score ≥ 50`,
      );
    if (numbers.banned > 0)
      issues.push(
        `${numbers.banned} banned number(s) — immediate attention required`,
      );
    ...
    const fpSkipRate =
      totals.sent > 0 ? fingerprintSkips.length / totals.sent : 0;
    if (fpSkipRate > 0.3)
      issues.push(
        `High FINGERPRINT_SKIP rate: ${fingerprintSkips.length} skips / ${totals.sent} sends — add more template diversity`,
      );
    if (hardLimitHits.length > days * 3)
      issues.push(
        `Frequent HARD_LIMIT_HIT: ${hardLimitHits.length} events — audience is outgrowing number capacity, consider activating second number`,
      );
    ...
    return {
      ...
      events: {
        ...
        warning_reasons: [...deliveryWarnings, ...readWarnings]
          .map((r: any) => r.reason ?? '')
          .filter(Boolean),
      ...
      audience: {
        ...
        high_fatigue_pct:
          total > 0 ? Math.round((highFatigue / total) * 100) : 0,
      },
      numbers,
      ...
      return {
        ...
        reply_rate_pct:
          sent + delivered > 0
            ...
            : 0,
        fail_rate_pct:
          attempted > 0 ? Math.round((failed / attempted) * 100) : 0,
      };
    });
  }
  ...
  private async _getNumberHealth(): Promise<{
    ...
    const avgRisk =
      rows.length > 0
        ? Math.round(
            rows.reduce((s, n) => s + Number(n.risk_score), 0) / rows.length,
          )
        : 0;
