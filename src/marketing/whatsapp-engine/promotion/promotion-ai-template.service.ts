...
const DISPLAY_TERMS =
  /display|panel|tray|holder|rack|counter|wall|shelf|stand|fixture|mount|mirror|cabinet|sunglass\s*display|frame\s*display|lens\s*tray|display\s*furniture|storage\s*unit/i;
const MACHINE_TERMS =
  /refractometer|phoropter|slit\s*lamp|edger|lens\s*edger|autoref|auto\s*ref|lensometer|focimeter|keratometer|tonometer|perimeter|visual\s*field|instrument|equipment|machine/i;
const CASE_TERMS =
  /case|eva\s*case|metal\s*case|hard\s*case|soft\s*case|spectacle\s*case|eyewear\s*case|frame\s*case|glasses\s*case|pouch|box/i;
const CLEANER_TERMS =
  /lens\s*cleaner|cleaning\s*kit|cleaning\s*spray|cleaning\s*solution|lens\s*spray|lens\s*solution|cleaner\s*kit|optical\s*cleaner/i;
const MICROFIBER_TERMS =
  /microfiber|micro\s*fiber|cleaning\s*cloth|gsm\s*cloth|optical\s*cloth|lens\s*cloth|cleaning\s*towel/i;
const TOOL_TERMS =
  /screw|screwdriver|plier|fitting\s*tool|nose\s*pad|temple|hinge|repair\s*kit|optical\s*tool|frame\s*tool|adjustment\s*tool|pd\s*ruler|pupillometer|height\s*gauge|pantoscopic|fitting\s*kit/i;
  ...
  switch (productClass) {
    case 'display':
      return {
        ...
        usageScenario:
          'organizing frame and product display across counters and wall panels',
        businessValue:
          'more active inventory, fewer products in dead storage, better customer self-navigation',
        conversationAngle:
          'how many products they currently display versus what they actually carry',
      };
    case 'machine':
      return {
        businessProblem:
          'patient throughput bottlenecks and outsourcing dependency for lens work',
        usageScenario: use ?? 'examination and in-house lens processing',
        businessValue:
          'faster patient flow, in-house quality control, reduced outsourcing cost',
        conversationAngle:
          'what examination instruments they use and what work they currently outsource',
      };
    case 'case':
      return {
        businessProblem:
          'lens damage complaints and generic handover experience',
        usageScenario:
          'packaging and protecting dispensed eyewear at point of sale',
        businessValue:
          'fewer post-sale complaints, higher perceived value, brand recall through daily use',
        conversationAngle:
          'what case type they currently use at handover and whether it matches their pricing',
      };
    case 'lens_cleaner':
      return {
        businessProblem:
          'coating damage from improper cleaning and missed repeat purchase opportunities',
        usageScenario: 'daily lens care and maintenance for dispensed eyewear',
        businessValue:
          'longer lens life, fewer coating complaints, repeat purchases at the counter',
        conversationAngle:
          'whether they provide cleaning products at handover and where customers currently buy them',
      };
    case 'microfiber':
      return {
        businessProblem:
          'coating abrasion from wrong cleaning materials and weak handover experience',
        usageScenario: 'daily lens surface cleaning and care',
        businessValue:
          'protected lens coatings, premium handover feel, branded daily-use touchpoint',
        conversationAngle:
          'what cloth they currently dispense and whether they are branding it',
      };
    case 'tool':
      return {
        businessProblem:
          'inaccurate fittings and outsourced repairs that should be resolved in-house',
        usageScenario:
          'frame fitting, adjustment, and in-house repair at the dispensing counter',
        businessValue:
          'faster fittings, consistent accuracy, fewer return visits from poor adjustments',
        conversationAngle:
          'what tools they currently have at each counter and what repairs they send out',
      };
    default:
      return {
        businessProblem:
          'fragmented supply and unmet customer requests from limited accessory range',
        usageScenario: use ?? 'regular optical business operations',
        businessValue:
          'fewer customer walk-aways, consolidated supply, more revenue per customer',
        conversationAngle:
          'what accessories they carry and how many suppliers they currently manage',
      };
  }
}
...
function deriveProductType(words: string[]): string {
  ...
  const candidates = words.filter(
    (w) => !stop.has(w) && w.length > 2 && !/^\d/.test(w),
  );
...
function extractProductKnowledge(
  ...
  const allText = layers
    .map(([t]) => t)
    .concat(product.sku?.toLowerCase() ?? '')
    .join(' ');
  ...
  if (!industryContext && product.hsnCode && productClass !== 'display') {
    ...
    if (mapped) {
      industryContext = mapped;
      hsnFallbackUsed = true;
    }
  }
  ...
  const targetUser = industryContext
    ? (TARGET_USER_MAP[industryContext] ?? null)
    : null;
  const enrich = buildEnrichSentence(
    productType,
    primaryUse,
    material,
    industryContext,
...
export function scoreMessage(
  result: PromotionGenerateResult,
): MessageQualityScore {
  ...
  if (wordCount > 70) {
    ...
    naturalLanguage -= 1;
    flags.push('message_approaching_limit');
  } else if (wordCount >= 35 && wordCount <= 55) {
    naturalLanguage += 1;
  } // ideal range bonus
  ...
  const longSentences = sentences.filter(
    (s) => s.split(/\s+/).filter(Boolean).length > 12,
  ).length;
  if (longSentences >= 2) {
    naturalLanguage -= 2;
    flags.push('sentence_too_long');
  ...
  const hasExplanationTooLong = bodyParagraphs.some(
    (p) => (p.match(/[.!?]/g) ?? []).length > 1,
  );
  if (hasExplanationTooLong) {
    naturalLanguage -= 1;
    flags.push('explanation_too_long');
  }

  for (const p of CATALOG_PATTERNS) {
    if (p.test(message)) {
      naturalLanguage -= 2;
      flags.push(`catalog_pattern:${p.toString()}`);
      break;
    }
  }
  for (const p of FORBIDDEN_PATTERNS) {
    if (p.test(message)) {
      naturalLanguage -= 3;
      flags.push(`forbidden:${p.toString()}`);
      break;
    }
  }
  ...
  if (!hook || hook.trim() === '') {
    naturalLanguage -= 3;
    flags.push('missing_hook');
  } else {
    if (hookFlags.includes('hook_too_long')) {
      naturalLanguage -= 1;
      flags.push('hook_too_long');
    }
    if (hookFlags.includes('hook_contains_product')) {
      naturalLanguage -= 2;
      flags.push('hook_contains_product');
    }
    if (hookFlags.includes('hook_contains_greeting')) {
      naturalLanguage -= 2;
      flags.push('hook_contains_greeting');
    }
    if (hookFlags.includes('weak_hook')) {
      naturalLanguage -= 1;
      flags.push('weak_hook');
    }
  }
  ...
  // ── Business Value ────────────────────────────────────────────────────────
  let businessValue = 4;
  if (['problem_based', 'workflow_based'].includes(metadata.contentCategory))
    businessValue += 2;
  ...
  const hasForbidden = flags.some(
    (f) => f.startsWith('forbidden') || f.startsWith('catalog'),
  );
...
export class PromotionAiTemplateService {
  ...
  async generate(
    input: PromotionGenerateInput,
  ): Promise<PromotionGenerateResult> {
    ...
    for (let attempt = 0; attempt < MAX_REGENERATION_ATTEMPTS; attempt++) {
      ...
      const message = this._buildMessage({
        ...
        callLabel,
        telecallerPhone: telecaller_phone,
        productUrl,
        offerLine,
      });
      ...
      result.quality = {
        finalScore: lastScore.total,
        attemptsUsed: attempt + 1,
        grade: lastScore.grade,
      };

      if (
        lastScore.total >= QUALITY_THRESHOLD &&
        !lastScore.flags.some((f) => f.startsWith('forbidden'))
      ) {
  ...
  async generateValidationReport(sampleCount = 100): Promise<{
    ...
    const testProducts = [
      {
        id: 1,
        itemName: 'Frame Display Wall Panel',
        sku: 'DP001',
        productType: 'display',
        description: 'optical frame display wall panel',
        handle: 'frame-display-wall-panel',
        image: 'https://cdn.shopify.com/test.jpg',
        syncIgnored: false,
      },
      {
        id: 2,
        itemName: 'Auto Refractometer',
        sku: 'MR001',
        productType: 'optical instrument',
        description: 'auto refractometer optical instrument',
        handle: 'auto-refractometer',
        image: 'https://cdn.shopify.com/test.jpg',
        syncIgnored: false,
      },
      {
        id: 3,
        itemName: 'EVA Spectacle Case',
        sku: 'EC001',
        productType: 'spectacle case',
        description: 'eva spectacle case optical',
        handle: 'eva-spectacle-case',
        image: 'https://cdn.shopify.com/test.jpg',
        syncIgnored: false,
      },
      {
        id: 4,
        itemName: 'Lens Cleaning Spray 100ml',
        sku: 'LC001',
        productType: 'lens cleaner',
        description: 'optical lens cleaning spray',
        handle: 'lens-cleaning-spray',
        image: 'https://cdn.shopify.com/test.jpg',
        syncIgnored: false,
      },
      {
        id: 5,
        itemName: 'Optical Microfiber Cloth',
        sku: 'MC001',
        productType: 'microfiber cloth',
        description: 'optical microfiber cleaning cloth',
        handle: 'optical-microfiber-cloth',
        image: 'https://cdn.shopify.com/test.jpg',
        syncIgnored: false,
      },
      {
        id: 6,
        itemName: 'Optical Screwdriver Set',
        sku: 'TS001',
        productType: 'optical tool',
        description: 'optical screwdriver fitting tool',
        handle: 'optical-screwdriver-set',
        image: 'https://cdn.shopify.com/test.jpg',
        syncIgnored: false,
      },
    ...
    const totalWords = results.reduce(
      (sum, r) => sum + r.message.split(/\s+/).filter(Boolean).length,
      0,
    );
    const avgWordCount = parseFloat((totalWords / results.length).toFixed(1));

    const allSentences = results.flatMap((r) =>
      r.message.split(/[.!?]\s+/).filter(Boolean),
    );
    ...
    const avgSentenceLength = parseFloat(
      (totalSentenceWords / allSentences.length).toFixed(1),
    );

    const scores = results.map((r) => scoreMessage(r));
    const avgReplyPotential = parseFloat(
      (
        scores.reduce((sum, s) => sum + s.conversationPotential, 0) /
        scores.length
      ).toFixed(1),
    );
    const passRate = parseFloat(
      (
        (scores.filter((s) => s.grade === 'PASS').length / scores.length) *
        100
      ).toFixed(1),
    );
    ...
    const rejectionBreakdown = {
      ...
      missingQuestions: scores.filter((s) =>
        s.flags.includes('missing_question'),
      ).length,
      longMessages: scores.filter((s) => s.flags.includes('message_too_long'))
        .length,
      corporateLanguage: scores.filter((s) =>
        s.flags.some((f) => f.startsWith('forbidden:')),
      ).length,
    };
  ...
  private _categoryBlock(
    category: ContentCategory,
    pools: ProductClassPools,
  ): ContentBlock {
  ...
  private _buildProductUrl(product: ShopifyCatalogItem): {
    productUrl: string;
    urlSource: 'handle' | 'search' | 'rejected';
  } {
  ...
  private _pickHook(productClass: ProductClass, phone?: string): string {
    ...
    return this._pickFromPool(pool, this._hookHistory, phone);
  }
  ...
  private _validateHook(
    ...
    if (/\b(hi|hello|dear|greetings|namaste)\b/i.test(hook))
      f.push('hook_contains_greeting');
    ...
    if (
      ...
      if (hookLower.includes(productName.toLowerCase()))
        f.push('hook_contains_product');
    }
  ...
  private _pickFromPool<T extends string>(
    ...
    const picked = this._pickRandom(source);
    history.set(
      phone,
      [...(history.get(phone) ?? []), picked].slice(-pool.length),
    );
    return picked;
  }
  ...
  private _assertSafe(message: string): void {
    for (const pattern of FORBIDDEN_PATTERNS) {
      if (pattern.test(message)) {
        this.logger.warn(
          `[PROMO_AI_SAFETY] blocked_pattern=${pattern.toString()}`,
        );
      }
    }
    ...
    if (emojiCount > 3) {
      this.logger.warn(
        `[PROMO_AI_SAFETY] emoji_count=${emojiCount} exceeds limit`,
      );
    }
  }
}
