...
export class QueueService {
  ...
  async findPending(limit = 5): Promise<WhatsappMessageQueue[]> {
    ...
    const results = await this.repo.find({
      where: { status: QueueStatus.PENDING, scheduled_at: LessThanOrEqual(now) },
    ...
    const [totalPending, futurePending, minRow, statusDist] = await Promise.all([
      ...
      this.repo.createQueryBuilder('q')
  ...
  async assignSender(id: string, numberId: string, phone: string, name: string): Promise<void> {
  ...
  async patchPayload(id: string, patch: Record<string, unknown>): Promise<void> {
    const row = await this.repo.findOne({ where: { id }, select: ['id', 'message_payload'] });
    if (!row) return;
    await this.repo.update(id, {
  ...
  async markDeferred(id: string, reason: string, deferMs = 60 * 60_000): Promise<void> {
    const deferUntil = new Date(Date.now() + deferMs);
    this.logger.log(`[MKT_QUEUE_STATUS_UPDATE] id=${id} old=PROCESSING new=PENDING(deferred) until=${deferUntil.toISOString()} reason="${reason.slice(0, 80)}"`);
    await this.repo.update(id, {
  ...
  async buildFromCampaign(campaign: MarketingCampaign): Promise<number> {
    ...
    const todayTotal = await this.repo.count({
      where: { campaign_id: campaign.id, created_at: MoreThanOrEqual(todayStart) },
    });
