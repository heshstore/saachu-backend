...
export class LeadAutomationService {
  ...
  private _runningRemindOverdueFollowUps = false;
  private _runningRecoverMissedLeads = false;
  private _runningNudgeQuotationPending = false;
  private _runningFollowUpWithCustomerOnQuotation = false;
  private _runningAlertHotLeadNotContacted = false;
  private _runningAlertUnansweredCustomerReply = false;
  private _runningResumeExpiredSnoozes = false;
  private _runningPaymentFollowUps = false;
  private _runningDetectStagnantLeads = false;
  private _runningAutoPriorityElevation = false;
  private _runningAutoReassignStaleLeads = false;
  private _runningReactivationEngine = false;
  ...
  private readonly waFailureMap = new Map<
    number,
    {
      count: number;
  ...
  private isWaBlocked(userId: number): boolean {
    ...
    if (Date.now() > entry.blockedUntil) {
      this.waFailureMap.delete(userId);
      return false;
    }
    return true;
  }
  ...
  private recordWaFailure(userId: number): void {
    ...
    if (entry.count >= 3 && entry.blockedUntil <= now) {
      ...
      this.logger.warn(
        `[WA_SUPPRESS] user=${userId} blocked 30m (${entry.count} failures in window)`,
      );
    }
  ...
  async handleLeadCreated(ev: LeadCreatedEvent): Promise<void> {
    ...
    if (
      ev.phone &&
      AUTO_REPLY_SOURCES.includes(ev.source) &&
      (await this.isSetting('automation.lead_greeting'))
    ) {
  ...
  async handleLeadEscalated(ev: LeadEscalatedEvent): Promise<void> {
    ...
    if (await this.wasRecentlyEscalated(ev.id, 'MANAGER_REVIEW', 720)) {
      this.logger.debug(
        `[ESC] Lead ${ev.id} — MANAGER_REVIEW cooldown active, skipping`,
      );
      return;
    }
    ...
    this.logger.log(
      `[ESC] handleLeadEscalated: notified ${managers.length} manager(s) for lead ${ev.id} — ${ev.reason}`,
    );
  }
  ...
  private async isSetting(key: string, defaultOn = true): Promise<boolean> {
    const cached = this._settingCache.get(key);
    if (cached && cached.expiresAt > Date.now()) return cached.value;
    const rows: { value: string }[] = await this.ds.query(
      `SELECT value FROM crm_settings WHERE key = $1 LIMIT 1`,
      [key],
    );
    const value = rows.length ? rows[0].value !== 'false' : defaultOn;
    this._settingCache.set(key, { value, expiresAt: Date.now() + 300_000 });
    return value;
  }
  ...
  async remindOverdueFollowUps(): Promise<void> {
    if (this._runningRemindOverdueFollowUps) return;
    this._runningRemindOverdueFollowUps = true;
    try {
      ...
      for (const lead of leads) {
        if (
          await this.wasRecentlyEscalated(
            lead.id,
            'SLA_BREACH',
            COOLDOWNS.SLA_BREACH,
          )
        )
          continue;

        const overdueMs =
          Date.now() - new Date(lead.next_action_due_at).getTime();
        ...
        const isA = ['SEND_QUOTATION', 'CHASE_QUOTATION'].includes(
          lead.workflow_state,
        );
        ...
        this.logger.log(
          `[SLA] Lead ${lead.id} overdue by ${Math.round(overdueH)}h [${lead.workflow_state}]`,
        );
      ...
      for (const lead of callbackBreaches) {
        ...
        this.logger.log(
          `[SLA] Lead ${lead.id} CALLBACK_WAIT breach — ${lead.name}`,
        );
      ...
      for (const lead of escLeadsFiltered) {
        ...
        this.logger.log(
          `[ESC] NO_ANSWER_ESC lead ${lead.id} — ${lead.no_answer_count} unanswered calls`,
        );
      }

      // ── Manager digest — ONE notification per manager per cycle ───────────────
      const totalIssues =
        slaBreachCount + callbackBreachCount + escLeadsFiltered.length;
      if (totalIssues > 0) {
        const managers: Array<{ id: number }> = await this.ds.query(
        ...
        if (callbackBreachCount > 0)
          digestLines.push(
            `• ${callbackBreachCount} callback breach${callbackBreachCount > 1 ? 'es' : ''}`,
          );
        if (escLeadsFiltered.length > 0)
          digestLines.push(
            `• ${escLeadsFiltered.length} NO_ANSWER escalation${escLeadsFiltered.length > 1 ? 's' : ''}`,
          );
        if (escLeadsFiltered.length > 0) {
          digestLines.push(
            ...escLeadsFiltered
              .slice(0, 3)
              .map((l) => `  – ${l.name} (${l.no_answer_count}× no answer)`),
          );
        }
      ...
      this._runningRemindOverdueFollowUps = false;
    }
  }
  ...
  async recoverMissedLeads(): Promise<void> {
    if (this._runningRecoverMissedLeads) return;
    this._runningRecoverMissedLeads = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
      const leads: Array<{
        id: number;
  ...
  async nudgeQuotationPending(): Promise<void> {
    if (this._runningNudgeQuotationPending) return;
    this._runningNudgeQuotationPending = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
      ...
      const leads: Array<{
        ...
        product_interest: string;
        assigned_to: number;
        workflow_state_entered_at: string;
      }> = await this.ds.query(
    ...
    `);
      ...
      for (const lead of leads) {
        ...
        if (
          await this.wasRecentlyEscalated(lead.id, 'QUOTATION_ESCALATION', 720)
        )
          continue;

        const hoursStale = Math.floor(
          (Date.now() - new Date(lead.workflow_state_entered_at).getTime()) /
            3_600_000,
        );
        this.logger.log(
          `[SLA] Lead ${lead.id} SEND_QUOTATION ${hoursStale}h stale — no quotation sent`,
        );
      ...
      if (quotationNudgeCount > 0) {
        const digestMsg = leads
          ...
          .map(
            (l) =>
              `• ${l.name} (${Math.floor((Date.now() - new Date(l.workflow_state_entered_at).getTime()) / 3_600_000)}h)`,
          )
          .join('\n');
        for (const manager of managers) {
          await this.notifService.createNotification({
        ...
        this.logger.log(
          `[DIGEST] quotation breach: ${quotationNudgeCount} lead(s) flagged to ${managers.length} manager(s)`,
        );
      }

      this.logger.log(
        `[SLA] nudgeQuotationPending: ${quotationNudgeCount}/${leads.length} SEND_QUOTATION lead(s) nudged`,
      );
    } catch (e: any) {
      this.dbHealth.handleError(e, 'LeadAutomation.nudgeQuotationPending');
    } finally {
      this._runningNudgeQuotationPending = false;
    }
  }
  ...
  async followUpWithCustomerOnQuotation(): Promise<void> {
    if (this._runningFollowUpWithCustomerOnQuotation) return;
    this._runningFollowUpWithCustomerOnQuotation = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
      ...
      this.logger.log(
        `followUpWithCustomerOnQuotation: sent follow-up to ${leads.length} customer(s)`,
      );
      await this.recordLastRun('cron.customer_quotation_followup.last_run');
    } catch (e: any) {
      this.dbHealth.handleError(
        e,
        'LeadAutomation.followUpWithCustomerOnQuotation',
      );
    } finally {
      this._runningFollowUpWithCustomerOnQuotation = false;
    }
  }
  ...
  private static readonly HOT_LEAD_WINDOWS: Record<
    string,
    { minH: number; maxH: number; emoji: string; label: string }
  > = {
  ...
  async alertHotLeadNotContacted(): Promise<void> {
    if (this._runningAlertHotLeadNotContacted) return;
    this._runningAlertHotLeadNotContacted = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
      ...
      for (const lead of leads) {
        const minsOld = Math.floor(
          (Date.now() - new Date(lead.created_at).getTime()) / 60_000,
        );
      ...
      this.logger.log(
        `alertHotLeadNotContacted: ${leads.length} hot lead(s) uncontacted`,
      );
    } catch (e: any) {
      this.dbHealth.handleError(e, 'LeadAutomation.alertHotLeadNotContacted');
    } finally {
      this._runningAlertHotLeadNotContacted = false;
    }
  }
  ...
  async alertUnansweredCustomerReply(): Promise<void> {
    if (this._runningAlertUnansweredCustomerReply) return;
    this._runningAlertUnansweredCustomerReply = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
      const leads: Array<{
        id: number;
      ...
      this.logger.log(
        `alertUnansweredCustomerReply: ${leads.length} unanswered reply/replies`,
      );
    } catch (e: any) {
      this.dbHealth.handleError(
        e,
        'LeadAutomation.alertUnansweredCustomerReply',
      );
    } finally {
      this._runningAlertUnansweredCustomerReply = false;
    }
  }
  ...
  async resumeExpiredSnoozes(): Promise<void> {
    if (this._runningResumeExpiredSnoozes) return;
    this._runningResumeExpiredSnoozes = true;
    try {
      ...
      if (result.length) {
        this.logger.log(
          `resumeExpiredSnoozes: resumed automation for ${result.length} lead(s)`,
        );
      }
    } catch (e: any) {
      this.dbHealth.handleError(e, 'LeadAutomation.resumeExpiredSnoozes');
    } finally {
      this._runningResumeExpiredSnoozes = false;
    }
  }
  ...
  async paymentFollowUps(): Promise<void> {
    if (this._runningPaymentFollowUps) return;
    this._runningPaymentFollowUps = true;
    try {
  ...
  async detectStagnantLeads(): Promise<void> {
    if (this._runningDetectStagnantLeads) return;
    this._runningDetectStagnantLeads = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
    ...
    `);
      ...
      for (const lead of staleLeads) {
        if (await this.wasRecentlyEscalated(lead.id, 'STALE_LEAD', 120))
          continue;

        this.logger.log(
          `[STALE] Lead ${lead.id} stagnant — no CALL in SLA×2 window [${lead.workflow_state}]`,
        );
    ...
    `);
      ...
      if (longStaleFiltered.length > 0) {
        ...
        const digestLines = longStaleFiltered
          .slice(0, 5)
          .map((l) => `• ${l.name} [${l.workflow_state}]`);
        ...
        this.logger.log(
          `[STALE] ${longStaleFiltered.length} long-stale lead(s) escalated to ${managers.length} manager(s)`,
        );
      }

      if (staleCount > 0 || longStaleFiltered.length > 0) {
        this.logger.log(
          `detectStagnantLeads: ${staleCount} new stale, ${longStaleFiltered.length} long-stale escalated`,
        );
      }
      ...
      this._runningDetectStagnantLeads = false;
    }
  }

  // ── Private helpers ───────────────────────────────────────────────────────────

  private async safeWaSendToUser(
    userId: number,
    message: string,
  ): Promise<void> {
    if (this.isWaBlocked(userId)) {
      this.logger.debug(
        `[WA_SUPPRESS] user=${userId} blocked — skipping WA send`,
      );
      return;
    }
    try {
      ...
      this.logger.warn(
        `[WA_SUPPRESS] user=${userId} WA send failed: ${e?.message}`,
      );
    }
  }

  private async safeWaSendToPhone(
    phone: string,
    message: string,
  ): Promise<void> {
  ...
  async autoPriorityElevation(): Promise<void> {
    if (this._runningAutoPriorityElevation) return;
    this._runningAutoPriorityElevation = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;

      const lowToMedium: Array<{
        id: number;
        name: string;
        assigned_to: number;
      }> = await this.ds.query(`
    ...
    `);
      ...
      this._runningAutoPriorityElevation = false;
    }
  }
  ...
  async autoReassignStaleLeads(): Promise<void> {
    if (this._runningAutoReassignStaleLeads) return;
    this._runningAutoReassignStaleLeads = true;
    try {
      if (!(await this.isSetting('automation.followup_reminders'))) return;
      ...
      for (const lead of stale) {
        ...
        `,
          ...
          this.logger.log(
            `[REASSIGN] Lead ${lead.id} reassigned from user=${oldAssigneeId} to user=${newAssigneeId}`,
          );
          ...
          this.logger.warn(
            `[REASSIGN] autoReassignStaleLeads failed for lead ${lead.id}: ${e?.message}`,
          );
        }
      }

      this.logger.log(
        `[REASSIGN] autoReassignStaleLeads: processed ${stale.length} stale lead(s)`,
      );
      await this.recordLastRun('cron.auto_reassign.last_run');
    } catch (e: any) {
      this.dbHealth.handleError(e, 'LeadAutomation.autoReassignStaleLeads');
    } finally {
      this._runningAutoReassignStaleLeads = false;
    }
  }
  ...
  async reactivationEngine(): Promise<void> {
    if (this._runningReactivationEngine) return;
    this._runningReactivationEngine = true;
    try {
