...
export const PERMISSION_DEPS: Record<string, string[]> = {
  ...
  'production.update_priority': ['production.view'],
  'production.analytics': ['production.view'],
  'production.decision_engine': [
    'production.view',
...
export class RbacService implements OnModuleInit {
  ...
  async getMatrix() {
    ...
    for (const role of roles) {
      const full = await this.roleRepo.findOne({
        where: { id: role.id },
  ...
  async saveMatrix(data: { roleId: number; permissionIds: number[] }[]) {
    ...
    for (const { roleId, permissionIds } of data) {
      ...
      const keys = permissionIds.map((id) => idToKey.get(id)).filter(Boolean);
  ...
  async createRole(name: string): Promise<Role> {
    ...
    if (RESERVED_ROLE_NAMES.has(trimmed)) {
      throw new BadRequestException(
        `Role name "${trimmed}" is reserved and cannot be created`,
      );
    }
  ...
  async renameRole(id: number, name: string): Promise<Role> {
    ...
    if (RESERVED_ROLE_NAMES.has(trimmed)) {
      throw new BadRequestException(
        `Role name "${trimmed}" is reserved and cannot be used`,
      );
    }
